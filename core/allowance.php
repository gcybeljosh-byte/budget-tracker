...
// RBAC Check
if (isset($_SESSION['permissions']) && is_array($_SESSION['permissions']) && isset($_SESSION['permissions']['manage_income']) && $_SESSION['permissions']['manage_income'] === false && $_SESSION['role'] !== 'superadmin') {
...
<div class="modal fade" id="sourceHistoryModal" tabindex="-1">
    <div class="modal-dialog modal-lg">
        <div class="modal-content border-0 rounded-4 shadow-lg">
            <div class="modal-header border-bottom-0 p-4 pb-0">
                <h5 class="modal-title fw-bold" id="historyModalTitle">Source History</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
            </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
                ...
                .then(data => {
                    if (data.success) {
                        ...
                        if (data.data.length === 0) {
                            historyTableBody.innerHTML = '<tr><td colspan="4" class="text-center text-muted py-4">No transactions found for this source.</td></tr>';
                            return;
                        }
                        data.data.forEach(item => {
                            ...
                            switch (item.type) {
                                case 'Allowance':
                                    typeBadge = '<span class="badge bg-success-subtle text-success">Allowance</span>';
                                    amountClass = 'text-success fw-bold';
                                    amountPrefix = '+';
                                    actionsHtml = `
                                    <button class="btn btn-sm btn-light text-primary me-1 rounded-circle" onclick="editAllowanceFromHistory(${item.id}, '${item.date}', '${item.description}', ${item.amount}, '${source}')">
                                        <i class="fas fa-edit"></i>
                                    </button>
                                    <button class="btn btn-sm btn-light text-danger rounded-circle" onclick="deleteAllowance(${item.id})">
                                        <i class="fas fa-trash"></i>
                                    </button>
                                `;
                                    break;
                                case 'Expense':
                                    typeBadge = '<span class="badge bg-danger-subtle text-danger">Expense</span>';
                                    amountClass = 'text-danger fw-bold';
                                    amountPrefix = '-';
                                    break;
                                case 'Savings':
                                    typeBadge = '<span class="badge bg-primary-subtle text-primary">To Savings</span>';
                            ...
                            <td class="text-end ${amountClass}">${amountPrefix}${formatCurrency(item.amount)}</td>
        ...
        // Proxy for history actions
        window.editAllowanceFromHistory = function(id, date, desc, amount, source) {
            openEditModal({
                id,
                date,
                desc,
            ...
            function startTutorial() {
                ...
                function showStep(index) {
                    if (index >= steps.length) {
                        markPageTutorialSeen('allowance.php');
                        return;
                    }
