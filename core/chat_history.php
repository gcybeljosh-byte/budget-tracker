...
foreach ($messages as $msg) {
    ...
    if (!isset($groupedMessages[$dateKey])) {
        ...
        if ($dateKey === $today) {
            ...
            $dateLabels[$dateKey] = date('F j, Y', strtotime($msg['created_at']));
        }
    }
    $groupedMessages[$dateKey][] = $msg;
}
            ...
            <div class="card-body p-0 overflow-auto flex-grow-1" id="chatBody" style="background: #fdfdfd; scroll-behavior: smooth;">
                ...
                <?php if (empty($messages)): ?>
                    <div class="h-100 d-flex flex-column align-items-center justify-content-center text-center p-5">
                        ...
                        <a href="<?php echo SITE_URL; ?>index.php" class="btn btn-primary rounded-pill px-4 mt-2">Go to Dashboard</a>
                    </div>
                <?php else: ?>
                    <div class="p-4">
                        <?php foreach ($groupedMessages as $date => $msgs): ?>
                            <div class="date-group-header text-center my-4 position-relative">
                                ...
                                <span class="bg-white px-3 text-secondary small fw-bold position-relative z-1"><?php echo $dateLabels[$date]; ?></span>
                            </div>
            ...
            if (!(window.seenTutorials && window.seenTutorials['chat_history.php'])) {
                const steps = [{
                    ...
                    },
                    {
                        ...
                        text: 'Use the search bar to quickly find any past message. Type a keyword and matching messages will be highlighted.',
                        target: '#chatSearch'
                    },
                    {
                        title: '📜 Conversation Log',
                        text: 'Your messages appear on the right (blue), and AI responses on the left (white). Scroll up to revisit any past advice or budget actions.',
                        target: '#chatBody'
                    },
                    {
                        title: '🗑️ Clear History',
                        text: 'Use the "Clear" button to permanently delete your chat history. Use this to start fresh, but note this cannot be undone.',
                        target: '.btn-outline-danger'
                    },
                    {
                        title: '🤖 Start a New Chat',
                        text: 'Tap the floating AI button (bottom-right of any page) anytime to ask questions, log transactions, or get budget advice!'
                    }
                ];
                ...
                fetch('<?php echo SITE_URL; ?>api/chat_history.php', {
                    ...
                    .then(data => {
                        if (data.success) {
                            ...
                            Swal.fire({
                                title: 'Error!',
                                text: data.message || 'Failed to delete history.',
                                icon: 'error',
                                confirmButtonColor: '#6366f1'
                            });
                        }
