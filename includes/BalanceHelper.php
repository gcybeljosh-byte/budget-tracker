...
class BalanceHelper
{
    ...
    public function getCashBalance($user_id, $monthOnly = false)
    {
        ...
        $sql = "
            SELECT 
                (SELECT COALESCE(SUM(amount), 0) FROM allowances WHERE user_id = ? AND source_type = 'Cash' AND (group_id IS NULL OR group_id = 0) $dateFilter) - 
                (SELECT COALESCE(SUM(amount), 0) FROM expenses WHERE user_id = ? AND source_type = 'Cash' AND expense_source = 'Allowance' AND (group_id IS NULL OR group_id = 0) $dateFilter) -
                (SELECT COALESCE(SUM(amount), 0) FROM savings WHERE user_id = ? AND source_type = 'Cash' AND (group_id IS NULL OR group_id = 0) $dateFilter) as balance
        ";
    ...
    public function getDigitalBalance($user_id, $monthOnly = false)
    {
        ...
        $sql = "
            SELECT 
                (SELECT COALESCE(SUM(amount), 0) FROM allowances WHERE user_id = ? AND source_type IN ('GCash', 'Maya', 'Bank', 'Electronic') AND (group_id IS NULL OR group_id = 0) $dateFilter) - 
                (SELECT COALESCE(SUM(amount), 0) FROM expenses WHERE user_id = ? AND source_type IN ('GCash', 'Maya', 'Bank', 'Electronic') AND expense_source = 'Allowance' AND (group_id IS NULL OR group_id = 0) $dateFilter) -
                (SELECT COALESCE(SUM(amount), 0) FROM savings WHERE user_id = ? AND source_type IN ('GCash', 'Maya', 'Bank', 'Electronic') AND (group_id IS NULL OR group_id = 0) $dateFilter) as balance
        ";
        $stmt = $this->conn->prepare($sql);
        $stmt->bind_param("iii", $user_id, $user_id, $user_id);
    ...
    public function getBalanceDetails($user_id, $expense_source, $source_type)
    {
        ...
        if ($expense_source === 'Savings') {
            $sql = "
                SELECT 
                    (SELECT COALESCE(SUM(amount), 0) FROM savings WHERE user_id = ? AND source_type = ? $dateFilter $groupFilter) as total_saved,
                    (SELECT COALESCE(SUM(amount), 0) FROM expenses WHERE user_id = ? AND source_type = ? AND expense_source = 'Savings' $dateFilter $groupFilter) as total_spent
            ";
            $stmt = $this->conn->prepare($sql);
            $stmt->bind_param("isis", $user_id, $source_type, $user_id, $source_type);
            $stmt->execute();
            ...
            $sql = "
                SELECT 
                    (SELECT COALESCE(SUM(amount), 0) FROM allowances WHERE user_id = ? AND source_type = ? $dateFilter $groupFilter) as total_allowance,
                    (SELECT COALESCE(SUM(amount), 0) FROM expenses WHERE user_id = ? AND source_type = ? AND expense_source = 'Allowance' $dateFilter $groupFilter) as total_expense,
                    (SELECT COALESCE(SUM(amount), 0) FROM savings WHERE user_id = ? AND source_type = ? $dateFilter $groupFilter) as total_savings
            ";
            $stmt = $this->conn->prepare($sql);
    ...
    public function syncBudgetLimits($user_id)
    {
        ...
        $stmt = $this->conn->prepare("SELECT COALESCE(SUM(amount), 0) as total FROM allowances WHERE user_id = ? AND (group_id IS NULL OR group_id = 0) AND date >= DATE_FORMAT(NOW(), '%Y-%m-01')");
